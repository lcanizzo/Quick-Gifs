...
//Set rating
    //Set on enter:
    $("#rated").on("keyup", function (e) {
        let input = $(this).val().trim().toUpperCase();
        console.log('inputRated:', input);
        //If "enter" pressed, change rating
        if (e.which == 13) {
            if (input == "Y" 
            ||
            input == "G"
            ...
            input == "R") {
                ...
                alert("Please enter a valide rating: Y, G, PG, PG-13, R");
            }
        }
    })
    //Set on click:
    $("#rated-button").on("click", function () {
        let input = $("#rated").val().trim().toUpperCase();
        console.log('inputRated:', input);        
        //If "O.K" pressed, change rating
        if (input == "Y" 
        ||
        input == "G"
        ||
